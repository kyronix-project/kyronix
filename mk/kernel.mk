...
KERNEL_BUILD_CONFIG := $(BUILD)/generated/kernel-config.h
INSTRUMENT ?= 0
...
$(KERNEL): $(KERNEL_OBJS) $(KALLSYMS_OBJ) linker.ld $(KERNEL_CONFIG) $(KERNEL_BUILD_CONFIG)
	@mkdir -p $(@D)
	$(LD) $(LDFLAGS) -o $@ $(KERNEL_OBJS) $(KALLSYMS_OBJ)

$(KERNEL_PRE): $(KERNEL_OBJS) $(KALLSYMS_SEED_OBJ) linker.ld $(KERNEL_CONFIG) $(KERNEL_BUILD_CONFIG)
	@mkdir -p $(@D)
	$(LD) $(LDFLAGS) -o $@ $(KERNEL_OBJS) $(KALLSYMS_SEED_OBJ)

$(BUILD)/%.o: %.c $(KERNEL_CONFIG) $(KERNEL_BUILD_CONFIG)
	@mkdir -p $(@D)
...
$(VIRTIO_NET_MODULE): kernel/drivers/virtio_net.c kernel/module.h $(KERNEL_CONFIG) $(KERNEL_BUILD_CONFIG)
	@mkdir -p $(@D)
...
$(KALLSYMS_OBJ): $(KALLSYMS_SRC) $(KERNEL_CONFIG) $(KERNEL_BUILD_CONFIG)
	@mkdir -p $(@D)
...
$(KERNEL_BUILD_CONFIG): FORCE
	@mkdir -p $(@D)
	@{ \
	    echo "/* Generated by mk/kernel.mk. */"; \
	    if [ "$(INSTRUMENT)" = "1" ]; then \
	        echo "#define CONFIG_KMEMLEAK 1"; \
	        echo "#define CONFIG_PROFILER 1"; \
	    fi; \
	} > $@.tmp
	@cmp -s $@.tmp $@ || mv $@.tmp $@
	@rm -f $@.tmp
