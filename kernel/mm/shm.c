...
int sys_shmget(int key, uint64_t size, int flags)
{
    ...
    shm_seg_t* slot = NULL;
    for (int i = 0; i < SHM_MAX_SEGS; i++)
        if (!g_segs[i].shmid) { slot = &g_segs[i]; break; }
...
uint64_t sys_shmat(int shmid, uint64_t shmaddr, int shmflg)
{
    ...
    for (int i = 0; i < SHM_MAX_ATTACH; i++)
        if (!g_attaches[i].shmid) { slot = &g_attaches[i]; break; }
    ...
    for (int i = 0; i < s->n_pages; i++) {
        if (vmm_map(p->space, va + (uint64_t)i * PAGE_SIZE,
                    (uint64_t)s->pages[i], vf) < 0) {
            for (int j = 0; j < i; j++)
...
int sys_shmctl(int shmid, int cmd, void* buf)
{
    ...
    switch (cmd) {
    case IPC_RMID:
        if (s->ref_count > 0) s->destroy_pending = 1;
        else seg_destroy(s);
        return 0;
    case IPC_STAT:
