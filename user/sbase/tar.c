...
static const char* filtertools[] = {
    ['J'] = "xz", ['Z'] = "compress", ['a'] = "lzma", ['j'] = "bzip2", ['z'] = "gzip",
};

static void pushdirtime(char* name, time_t mtime)
{
...
static struct dirtime* popdirtime(void)
{
    if (dirtimeslen)
    {
        dirtimeslen--;
...
static int comp(int fd, const char* tool, const char* flags)
{
    int fds[2];
...
static int decomp(int fd, const char* tool, const char* flags)
{
    int fds[2];
...
static ssize_t eread(int fd, void* buf, size_t n)
{
    ssize_t r;
...
static ssize_t ewrite(int fd, const void* buf, size_t n)
{
    ssize_t r;
...
static unsigned chksum(struct header* h)
{
    unsigned sum, i;
...
static void putoctal(char* dst, unsigned num, int size)
{
    if (snprintf(dst, size, "%.*o", size - 1, num) >= size)
        eprintf("putoctal: input number '%o' too large\n", num);
}

static int archive(const char* path)
{
    static const struct header blank = {
        "././@LongLink", "0000600", "0000000", "0000000", "00000000000", "00000000000",
        "       ",       AREG,      "",        "ustar",   "00",
    };
...
static int unarchive(char* fname, ssize_t l, char b[BLKSIZ])
{
    struct header* h = (struct header*) b;
    ...
    case HARDLINK:
    case SYMLINK:
        snprintf(lname, sizeof(lname), "%.*s", (int) sizeof(h->linkname), h->linkname);
...
static int print(char* fname, ssize_t l, char b[BLKSIZ])
{
    puts(fname);
    skipblk(l);
    return 0;
...
static void c(int dirfd, const char* name, struct stat* st, void* data, struct recursor* r)
{
    archive(r->path);
    if (vflag)
        puts(r->path);
...
static void sanitize(struct header* h)
{
    size_t i, j, l;
    struct
    {
        char* f;
        size_t l;
    } fields[] = {{h->mode, sizeof(h->mode)},   {h->uid, sizeof(h->uid)},
                  {h->gid, sizeof(h->gid)},     {h->size, sizeof(h->size)},
                  {h->mtime, sizeof(h->mtime)}, {h->chksum, sizeof(h->chksum)},
                  {h->major, sizeof(h->major)}, {h->minor, sizeof(h->minor)}};
...
static void chktar(struct header* h)
{
    const char* reason;
...
static void xt(int argc, char* argv[], int mode)
{
    long size;
