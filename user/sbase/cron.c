...
static void runjob(char* cmd)
{
    ...
    TAILQ_FOREACH(je, &jobhead, entry)
    {
        ...
        {
            loginfo("already running %s pid: %d at %s", je->cmd, je->pid, ctime(&t));
            return;
        }
    }
    ...
    {
    case -1:
        logerr("error: failed to fork job: %s time: %s", cmd, ctime(&t));
        return;
    case 0:
        setsid();
        loginfo("run: %s pid: %d at %s", cmd, getpid(), ctime(&t));
        execl("/bin/sh", "/bin/sh", "-c", cmd, (char*) NULL);
        logerr("error: failed to execute job: %s time: %s", cmd, ctime(&t));
        _exit(1);
    default:
...
static void waitjob(void)
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
    {
        ...
        if (WIFEXITED(status) == 1)
            loginfo("complete: pid: %d returned: %d time: %s", pid, WEXITSTATUS(status), ctime(&t));
        else if (WIFSIGNALED(status) == 1)
            loginfo("complete: pid: %d terminated by signal: %s time: %s", pid,
                    strsignal(WTERMSIG(status)), ctime(&t));
        else if (WIFSTOPPED(status) == 1)
            loginfo("complete: pid: %d stopped by signal: %s time: %s", pid,
                    strsignal(WSTOPSIG(status)), ctime(&t));
    }
}
...
static int daysinmon(int mon, int year)
{
...
static int matchentry(struct ctabentry* cte, struct tm* tm)
{
    struct
    {
        struct field* f;
...
static int parsefield(const char* field, long low, long high, struct field* f)
{
    int i;
    char *e1, *e2;
    const char* p;
...
static void freecte(struct ctabentry* cte, int nfields)
{
    switch (nfields)
    {
    case 6:
...
static void unloadentries(void)
{
...
static void reloadentries(void)
{
    unloadentries();
    if (loadentries() < 0)
