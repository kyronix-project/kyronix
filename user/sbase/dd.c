...
static size_t parsesize(char* expr)
{
    char* s = expr;
    size_t n = 1;
...
static void bswap(unsigned char* buf, size_t len)
{
    int c;
...
static void lcase(unsigned char* buf, size_t len)
{
    for (; len > 0; buf++, len--)
        buf[0] = tolower(buf[0]);
}

static void ucase(unsigned char* buf, size_t len)
{
    for (; len > 0; buf++, len--)
        buf[0] = toupper(buf[0]);
}
