...
static int nextln(int line)
{
    ++line;
    return (line > lastln) ? 0 : line;
}

static int prevln(int line)
{
    --line;
    return (line < 0) ? lastln : line;
}

static String* copystring(String* s, char* from)
{
    size_t len;
    char* t;
...
static char* addchar(char c, String* s)
{
    size_t cap = s->cap, siz = s->siz;
    char* t = s->str;

    if (siz >= cap && (cap > SIZE_MAX - LINESIZE || (t = realloc(t, cap += LINESIZE)) == NULL))
        error("out of memory");
    t[siz++] = c;
    s->siz = siz;
    s->cap = cap;
...
static int makeline(char* s, int* off)
{
    struct hline* lp;
    size_t len;
    char* begin = s;
    ...
    else
    {
        while ((c = *s++) && c != '\n')
            ;
        len = s - begin;
        if ((lp->seek = lseek(scratch, 0, SEEK_END)) < 0 || write(scratch, begin, len) < 0)
        {
...
repeat:
    ...
    if (!csize || off < lasto || off - lasto >= csize)
    {
        ...
        if (lseek(scratch, block, SEEK_SET) < 0 || (n = read(scratch, buf, CACHESIZ)) < 0)
        {
            error("input/output error");
        }
        csize = n;
...
static void setglobal(int i, int v)
{
...
static void newundo(int from1, int from2)
{
    struct link* p;
 ...
 *         from2 -> to2
 */
static void relink(int to1, int from1, int from2, int to2)
{
    newundo(from1, from2);
    zero[from1].next = to1;
    zero[from2].prev = to2;
...
static void inject(char* s, int where)
{
    int off, k, begin, end;
...
static void setscratch(void)
{
    ...
    if ((dir = getenv("TMPDIR")) == NULL)
        dir = "/tmp";
    r = snprintf(tmpname, sizeof(tmpname), "%s/%s", dir, "ed.XXXXXX");
...
static void deflines(int def1, int def2)
{
    if (!nlines)
    {
        line1 = def1;
...
static const char* expandcmd(void)
{
    static String cmd;
    char* p;
    int c, repl = 0;
...
static void dowrite(const char* fname, int trunc)
{
    size_t bytecount = 0;
    int i, r, line;
    FILE* aux;
...
static void doread(const char* fname)
{
    int r;
    size_t cnt;
    ssize_t len;
...
static int addsub(String* s, int nth, int nmatch)
{
    char *end, *q, *p, c;
    int sub;
...
static void subline(int num, int nth)
{
    int i, m, changed;
    static String s;
...
int main(int argc, char* argv[])
{
    ...
    if (!setjmp(savesp))
    {
        sigaction(SIGINT, &(struct sigaction) {.sa_handler = sigintr}, NULL);
        sigaction(SIGHUP, &(struct sigaction) {.sa_handler = sighup}, NULL);
