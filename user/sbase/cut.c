...
static size_t seek(struct line* s, size_t pos, size_t* prev, size_t count)
{
    ...
    else
    {
        ...
        {
            if ((s->len - i) >= delimlen && !memcmp(s->data + i, delim, delimlen))
            {
                if (!--n && count)
                    break;
                i += delimlen;
...
static void cut(FILE* fp, const char* fname)
{
    Range* r;
    struct line s;
