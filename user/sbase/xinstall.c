...
static void make_dir(char* dir, int was_missing)
{
...
static void make_dirs(char* dir, int was_missing)
{
    char* p;
...
static int install(const char* s1, const char* s2, int depth)
{
    int f1, f2;
...
{
    eprintf("usage: %s [-g group] [-o owner] [-m mode] (-d dir ... | [-D] (-t dest source ... | "
            "source ... dest))\n",
            argv0);
}
