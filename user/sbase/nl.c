...
static int getsection(struct line* l, int* section)
{
    size_t i;
    int sectionchanged = 0, newsection = *section;

    for (i = 0; (l->len - i) >= delimlen && !memcmp(l->data + i, delim, delimlen); i += delimlen)
    {
        if (!sectionchanged)
        {
            sectionchanged = 1;
...
static void nl(const char* fname, FILE* fp)
{
...
static char getlinetype(char* type, regex_t* preg)
{
    if (type[0] == 'p')
