...
static struct
{
    ...
    {"alnum", isalnumrune}, {"alpha", isalpharune}, {"blank", isblankrune},
    {"cntrl", iscntrlrune}, {"digit", isdigitrune}, {"graph", isgraphrune},
    {"lower", islowerrune}, {"print", isprintrune}, {"punct", ispunctrune},
    {"space", isspacerune}, {"upper", isupperrune}, {"xdigit", isxdigitrune},
};
...
static int check(Rune rune, unsigned checks)
{
    size_t i;
...
static size_t rangelen(struct range r)
{
    return (r.end - r.start + 1) * r.quant;
}

static size_t setlen(struct range* set, size_t setranges)
{
    size_t len = 0, i;
...
static int rstrmatch(Rune* r, char* s, size_t n)
{
    size_t i;
...
static size_t makeset(char* str, struct range** set, unsigned* checks)
{
    Rune* rstr;
    size_t len, i, j, m, n;
...
int main(int argc, char* argv[])
{
    ...
    if (!argc || argc > 2 || (dflag == sflag && argc != 2) || (dflag && argc != 1))
        usage();
    ...
    if (argc == 2)
    {
        ...
        if (set2checks && set2checks != ISLOWERBIT && set2checks != ISUPPERBIT)
        {
