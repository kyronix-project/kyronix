...
static void addpattern(const char* pattern)
{
    ...
    {
        sprintf(pnode->pattern, "%s%s%s%s%s", xflag ? "^" : "\\<", Eflag ? "(" : "\\(", pattern,
                Eflag ? ")" : "\\)", xflag ? "$" : "\\>");
    }
}

static void addpatternfile(FILE* fp)
{
...
static int grep(FILE* fp, const char* str)
{
    static char* buf = NULL;
    static size_t size = 0;
    ssize_t len = 0;
...
{
    enprintf(Error,
             "usage: %s [-EFHchilnqsvwx] [-e pattern] [-f file] "
             "[pattern] [file ...]\n",
             argv0);
}
