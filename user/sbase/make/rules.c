...
static Target* lookup(char* name)
{
    Target* tp;
...
static int depends(char* target, char* dep)
{
    int i;
...
void addtarget(char* target, int ndeps)
{
...
void adddep(char* target, char* dep)
{
    int i;
    size_t siz;
    Target **p, *tp = lookup(target);
...
static void freeaction(struct action* act)
{
    free(act->line);
    freeloc(&act->loc);
}

void addrule(char* target, struct action* acts, int n)
{
    int i;
    struct action* v;
    Target* tp = lookup(target);
...
static int execline(Target* tp, char* line, int ignore, int silence)
{
    char *s, *t;
    Target *p, **q;
...
static int touch(char* name, int ignore, int silence)
{
    char* cmd;
    int r, n;
...
static int touchdeps(Target* tp, int ignore, int silent)
{
    int r;
    Target** p;
...
static int enabled(char* suffix)
{
...
static Target* inference(Target* tp, int force)
{
    time_t t;
    int tolen, r;
    char *to, *from;
    ...
    for (p = suffixes->deps; p && *p; ++p)
    {
        ...
        r = snprintf(buf, sizeof(buf), "%s%s", from, to);
        ...
        r = snprintf(fname, sizeof(fname), "%*.*s%s", tolen, tolen, tp->name, from);

        if (r < 0 || r >= sizeof(fname))
        {
            error("prerequisite name too long %s %s", tp->name, from);
        }
...
static int rebuild(Target* tp, int* buildp)
{
    Target **p, *q;
    ;
    ...
    for (p = tp->deps; p && *p; ++p)
    {
        ...
        if (build)
        {
            debug("rebuild of %s forces rebuild of %s", q->name, tp->name);
            need = 1;
        }
        else if (q->stamp > tp->stamp)
        {
            debug("dependency %s is newer than %s", q->name, tp->name);
            need = 1;
        }
    }
    ...
    {
        debug("no action found for %s, looking a inference rule", tp->name);
