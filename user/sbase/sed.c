...
/* FIXME: move character inside Fninfo and only use 26*sizeof(Fninfo) instead of 127*sizeof(Fninfo)
 * bytes */
static Fninfo fns[] = {
    ['a'] = {cmd_a, get_aci_arg, free_acir_arg, 1}, /* schedule write of text for later */
    ['b'] = {cmd_b, get_bt_arg, NULL,
             2}, /* branch to label char *label when building, Cmd *jump when running */
    ['c'] = {cmd_c, get_aci_arg, free_acir_arg,
             2}, /* delete pattern space. at 0 or 1 addr or end of 2 addr, write text */
    ['d'] = {cmd_d, NULL, NULL, 2}, /* delete pattern space */
    ['D'] =
        {cmd_D, NULL, NULL,
         2}, /* delete to first newline and start new cycle without reading (if no newline, d) */
    ...
    ['n'] =
        {cmd_n, NULL, NULL,
         2}, /* write pattern space (unless -n) read to replace pattern space (if no input, quit) */
    ['N'] = {cmd_N, NULL, NULL, 2}, /* append to pattern space separated by newline, line number
                                       changes (if no input, quit) */
    ...
    ['r'] = {cmd_r, get_r_arg, free_acir_arg,
             1}, /* write contents of file (unable to open/read treated as empty file) */
    ...
    ['{'] = {cmd_lbrace, get_lbrace_arg, NULL,
             2}, /* if we match, run commands, otherwise jump to close */
    ['}'] = {cmd_rbrace, get_rbrace_arg, NULL,
             0}, /* noop, hold onto open for ease of building scripts */
...
static void resize(void** ptr, size_t* nmemb, size_t size, size_t new_nmemb, void** next)
{
    ...
    { /* turns out realloc(*ptr, 0) != free(*ptr) */
        free(*ptr);
        tmp = NULL;
    }
...
static char* find_delim(char* s, Rune delim, int do_brackets)
{
    enum
    {
        ...
        CLASS_CLOSING,    /* inside class [::], or colating element [..] or [==], and last character
                             was the respective : . or = */
    } state = OUTSIDE;
    ...
    for (; *s; s += rlen)
    {
        ...
        if (state == BRACKETS_OPENING && r == '^')
        {
            continue;
        }
        else if (state == BRACKETS_OPENING && r == ']')
        {
            state = BRACKETS_INSIDE;
            continue;
        }
        else if (state == BRACKETS_OPENING)
        {
            state = BRACKETS_INSIDE;
        }

        if (state == CLASS_CLOSING && r == ']')
        {
            state = BRACKETS_INSIDE;
        }
        else if (state == CLASS_CLOSING)
        {
            state = CLASS_INSIDE;
        }
        else if (state == CLASS_INSIDE && r == c)
        {
            state = CLASS_CLOSING;
        }
        else if (state == INSIDE_OPENING && (r == ':' || r == '.' || r == '='))
        {
            state = CLASS_INSIDE;
            c = r;
        }
        ...
        else if (state == INSIDE_OPENING)
        {
            state = BRACKETS_INSIDE;
        }
        else if (state == BRACKETS_INSIDE && r == '[')
        {
            state = INSIDE_OPENING;
        }
        else if (state == BRACKETS_INSIDE && r == ']')
        {
            state = OUTSIDE;
        }
        else if (state == OUTSIDE && escape)
        {
            escape = 0;
        }
        else if (state == OUTSIDE && r == '\\')
        {
            escape = 1;
        }
        else if (state == OUTSIDE && r == delim)
            return s;
        else if (state == OUTSIDE && do_brackets && r == '[')
        {
            state = BRACKETS_OPENING;
        }
    }
    return s;
}
...
static char* get_s_arg(Cmd* c, char* s)
{
    ...
    /* Find */
    if (!gflags.s_cont)
    { /* NOT continuing from literal newline in replacement text */
        lastre = 0;
...
static void cmd_l(Cmd* c)
{
    ...
    char* escapes[] = {
        /* FIXME: 7 entries and search instead of 127 */
        ['\\'] = "\\\\", ['\a'] = "\\a", ['\b'] = "\\b", ['\f'] = "\\f",
        ['\r'] = "\\r",  ['\t'] = "\\t", ['\v'] = "\\v", [0x7f] = NULL, /* fill out the table */
    };
...
static void cmd_s(Cmd* c)
{
    ...
    *genbuf.str = '\0';
    ...
    while (!qflag && !regexec(re, s, plen, pmatch, cflags))
    {
            ...
            (++matches == c->u.s.occurrence || !c->u.s.occurrence))
        {
            ...
            for (p = c->u.s.repl.str, len = strcspn(p, "\\&"); *p; len = strcspn(++p, "\\&"))
            {
                ...
                case '\\':
                    if (isdigit(*++p))
                    { /* backreference */
                        /* only need to check here if using lastre, otherwise we checked when
                         * building */
                    ...
                    { /* character after backslash taken literally (well one byte, but it works) */
                        strnacat(&genbuf, p, 1);
                    }
                    break;
                }
        ...
        if (!pmatch[0].rm_eo)
        { /* empty match, advance one rune and add it to output */
            end = s + strlen(s);
            rlen = charntorune(&r, s, end - s);

            if (!rlen)
            { /* ran out of bytes, copy short sequence */
                stracat(&genbuf, s);
                s = end;
            }
            else
            { /* copy whether or not it's a good rune */
...
static void cmd_y(Cmd* c)
{
    ...
    *genbuf.str = '\0';
    ...
    {
        if (!(rlen = charntorune(&r, s, end - s)))
        { /* ran out of chars, copy rest */
            stracat(&genbuf, s);
            break;
        }
        else if (r == Runeerror)
        { /* bad UTF-8 sequence, copy bytes */
            strnacat(&genbuf, s, rlen);
        }
        else
        {
            for (rp = c->u.y.set1; *rp; rp++)
                if (*rp == r)
                    break;
            if (*rp)
            { /* found r in set1, replace with Rune from set2 */
