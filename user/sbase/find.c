...
static int cmp_gt(int a, int b)
{
    return a > b;
}
static int cmp_eq(int a, int b)
{
    return a == b;
}
static int cmp_lt(int a, int b)
{
    return a < b;
}

/* order from find(1p), may want to alphabetize */
static struct pri_info primaries[] = {{"-name", pri_name, get_name_arg, NULL, 1},
...
static struct op_info ops[] = {{"(", LPAR, 0, 0, 0}, /* parens are handled specially */
...
static void parse(int argc, char** argv)
{
    ...
    for (arg = argv, tok = infix; *arg; arg++, tok++)
    {
        ...
        if (pri)
        { /* token is a primary, fill out tok and get arguments */
        ...
        else if ((op = find_op(*arg)))
        { /* token is an operator */
            if (lasttype == LPAR && op->type == RPAR)
                eprintf("empty parens\n");
            if ((lasttype == PRIM || lasttype == RPAR) && (op->type == NOT || op->type == LPAR))
            { /* need another implicit -a */
                *tok++ = and;
                ntok++;
            }
            tok->type = op->type;
...
static void find(char* path, struct findhist* hist)
{
    ...
    if (!S_ISDIR(st.st_mode) || gflags.prune || (gflags.xdev && hist && st.st_dev != hist->dev))
        return;
