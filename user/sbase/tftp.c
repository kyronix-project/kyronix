...
static int packreq(unsigned char* buf, int op, char* path, char* mode)
{
    unsigned char* p = buf;
...
static int packack(unsigned char* buf, int blkno)
{
    buf[0] = ACK >> 8;
    buf[1] = ACK & 0xff;
    buf[2] = blkno >> 8;
...
static int packdata(unsigned char* buf, int blkno)
{
    buf[0] = DATA >> 8;
    buf[1] = DATA & 0xff;
    buf[2] = blkno >> 8;
...
static int unpackop(unsigned char* buf)
{
    return (buf[0] << 8) | (buf[1] & 0xff);
}

static int unpackblkno(unsigned char* buf)
{
    return (buf[2] << 8) | (buf[3] & 0xff);
}

static int unpackerrc(unsigned char* buf)
{
    int errc;
...
static int writepkt(unsigned char* buf, int len)
{
    int n;

    n = sendto(s, buf, len, 0, (struct sockaddr*) &to, tolen);
    if (n < 0)
        if (errno != EINTR)
            eprintf("sendto:");
    return n;
...
static int readpkt(unsigned char* buf, int len)
{
    int n;

    n = recvfrom(s, buf, len, 0, (struct sockaddr*) &to, &tolen);
    if (n < 0)
    {
