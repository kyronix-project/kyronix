...
static size_t uniqskip(struct line* l)
{
    size_t i;
    int f = fskip, s = sskip;
...
static void uniqline(FILE* ofp, struct line* l)
{
    size_t loff;
    ...
    if (prevoff >= 0)
    {
        if ((prevlinecount == 1 && !dflag) || (prevlinecount != 1 && !uflag))
        {
            if (*countfmt)
...
static void uniq(FILE* fp, FILE* ofp)
{
...
static void uniqfinish(FILE* ofp)
{
    uniqline(ofp, NULL);
}
