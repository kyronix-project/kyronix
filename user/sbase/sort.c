...
static void skipblank(struct line* a)
{
...
static void skipnonblank(struct line* a)
{
    while (a->len && (*(a->data) != '\n' && *(a->data) != ' ' && *(a->data) != '\t'))
    {
        a->data++;
...
static void skipcolumn(struct line* a, int skip_to_next_col)
{
    char* s;
...
static void columns(struct line* line, const struct keydef* kd, struct column* col)
{
    Rune r;
...
static int skipmodcmp(struct line* a, struct line* b, int flags)
{
    Rune r1, r2;
...
static int slinecmp(struct line* a, struct line* b)
{
    int res = 0;
    double x, y;
    struct keydef* kd;
...
static int check(FILE* fp, const char* fname)
{
...
static int parse_flags(char** s, int* flags, int bflag)
{
    while (isalpha((int) **s))
    {
        switch (*((*s)++))
...
static void addkeydef(char* kdstr, int flags)
{
    struct keydef* kd;
...
{
    enprintf(2,
             "usage: %s [-Cbcdfimnru] [-o outfile] [-t delim] "
             "[-k def]... [file ...]\n",
             argv0);
}
