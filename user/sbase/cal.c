...
static int isleap(size_t year, enum caltype cal)
{
    if (cal == GREGORIAN)
    {
        if (year % 400 == 0)
...
static int monthlength(size_t year, int month, enum caltype cal)
{
...
static int dayofweek(size_t year, int month, int dom, enum caltype cal)
{
    size_t y;
    int m, a;
...
static void printgrid(size_t year, int month, int fday, int line)
{
    enum caltype cal;
...
static void drawcal(size_t year, int month, size_t ncols, size_t nmons, int fday)
{
    char* smon[] = {"January", "February", "March",     "April",   "May",      "June",
