...
static int digitsleft(const char* d)
{
    int shift;
    char* exp;
...
static int digitsright(const char* d)
{
    int shift, after;
    char* exp;
...
static int validfmt(const char* fmt)
{
    int occur = 0;
...
int main(int argc, char* argv[])
{
    ...
    if (fmt == ftmp)
    {
        right = MAX(digitsright(starts), MAX(digitsright(ends), digitsright(steps)));
