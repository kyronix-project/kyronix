...
static void swaplines(struct jline* la, struct jline* lb)
{
    struct jline tmp;
...
static void prjoin(struct jline* la, struct jline* lb, size_t jfa, size_t jfb)
{
    struct spec* sp;
...
static void prline(struct jline* lp)
{
...
static int jlinecmp(struct jline* la, struct jline* lb, size_t jfa, size_t jfb)
{
    int status;
...
static void addfield(struct jline* lp, char* sp, size_t len)
{
    if (lp->nf >= lp->maxf)
    {
        lp->fields = ereallocarray(lp->fields, (GROW * lp->maxf), sizeof(struct field));
        lp->maxf *= GROW;
    }
...
static void prspanjoin(struct span* spa, struct span* spb, size_t jfa, size_t jfb)
{
    size_t i, j;
...
static struct jline* makeline(char* s, size_t len)
{
    struct jline* lp;
    char* tmp;
    size_t i, end;
    ...
    while (i < lp->text.len)
    {
        if (sep)
        {
            if ((lp->text.len - i) < seplen ||
                !(tmp = memmem(lp->text.data + i, lp->text.len - i, sep, seplen)))
            {
                goto eol;
            }
...
static int addtospan(struct span* sp, FILE* fp, int reset)
{
    ...
    if (sp->nl >= sp->maxl)
    {
        sp->lines = ereallocarray(sp->lines, (GROW * sp->maxl), sizeof(struct jline*));
        sp->maxl *= GROW;
    }
...
static void addspec(struct outlist* olp, struct spec* sp)
{
    if (olp->ns >= olp->maxs)
    {
        olp->specs = ereallocarray(olp->specs, (GROW * olp->maxs), sizeof(struct spec*));
        olp->maxs *= GROW;
    }
    olp->specs[olp->ns] = sp;
    olp->ns++;
...
static struct spec* makespec(char* s)
{
    ...
    if (!strcmp(s, "0"))
    { /* join field must be 0 and nothing else */
        fileno = 0;
        fldno = 0;
    }
...
static void join(FILE* fa, FILE* fb, size_t jfa, size_t jfb)
{
    ...
    while (spa.nl && spb.nl)
    {
        if ((cmp = jlinecmp(spa.lines[0], spb.lines[0], jfa, jfb)) < 0)
        {
            ...
            {
                if (unpairsb)
                { /* a is EOF'd; print the rest of b */
                    do
                        prline(spb.lines[0]);
        ...
        else if (cmp > 0)
        {
            ...
            {
                if (unpairsa)
                { /* b is EOF'd; print the rest of a */
                    do
                        prline(spa.lines[0]);
        ...
        { /* FIELD_ERROR: both lines lacked join fields */
            if (unpairsa)
                prline(spa.lines[0]);
            if (unpairsb)
                prline(spb.lines[0]);
