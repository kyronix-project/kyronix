...
static void dumpnum(char* msg, Num* num)
{
    signed char* p;
...
static Num* moreroom(Num* num, int more)
{
    int ro, wo, room;
    signed char* p;
...
static Num* grow(Num* num)
{
    return moreroom(num, NDIGITS);
}

static Num* expand(Num* num, int min)
{
    if (min < num->room)
        return num;
...
static Num* zeronum(int ndigits)
{
    Num* num = new (ndigits);
...
static Num* wrdigit(Num* num, int d)
{
...
static Num* poke(Num* num, unsigned d)
{
...
static int begin(Num* num)
{
    return num->begin != 0;
}

static int first(Num* num)
{
    num->begin = 0;
    num->rp = num->buf;
...
static int more(Num* num)
{
    return (num->rp != num->wp);
}

static int length(Num* num)
{
    return num->wp - num->buf;
}

static int tell(Num* num)
{
    return num->rp - num->buf;
}

static void seek(Num* num, int pos)
{
    num->rp = num->buf + pos;
}

static void rshift(Num* num, int n)
{
    int diff;
...
static void lshift(Num* num, int n)
{
    int len;
...
static int negative(Num* num)
{
    return last(num) && peek(num) == -1;
}

static Num* norm(Num* n)
{
    /* trailing 0 */
...
static Num* mulnto(Num* src, Num* dst, int n)
{
    div_t dd;
    int d, carry;
...
static void div10(Num* num, int n)
{
    div_t dd = div(n, 2);
...
static void mul10(Num* num, int n)
{
    div_t dd = div(n, 2);
...
static Num* addn(Num* num, int val)
{
    int d, carry = val;
 ...
 */
static Num* divmod(Num* odivd, Num* odivr, Num** remp)
{
...
static int divscale(Num* divd, Num* divr)
{
    int diff;
...
static Num* expnum(Num* base, Num* exp)
{
    int neg, d;
...
static void printd(int d, int base, int space)
{
    int w, n;
...
static void pushdigit(struct digit** l, int val)
{
...
static int popdigit(struct digit** l)
{
    int val;
...
static void printnum(Num* onum, int base)
{
    struct digit* sp;
    int sc, i, sign, n;
...
static void addinput(FILE* fp, char* s)
{
    struct input* ip;
...
static void delinput(int cmd, int n)
{
    if (n < 0)
...
static void arith(Num* (*fn)(Num*, Num*) )
{
    Num *a, *b, *c;
...
static char* string(char* dst, int* np)
{
    int n, ch;
...
static struct reg* lookup(char* name)
{
    struct reg* rp;
