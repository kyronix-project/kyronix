...
static void printaddress(off_t addr)
{
    char fmt[] = "%07j#";
...
static void printchunk(const unsigned char* s, unsigned char format, size_t len)
{
...
static void printline(const unsigned char* line, size_t len, off_t addr)
{
    struct type* t = NULL;
    size_t i;
    int first = 1;
    ...
    once:
        ...
        for (i = 0; i < len; i += MIN(len - i, t ? t->len : 4))
        {
            if (len - i < (t ? t->len : 4))
            {
                ...
                printchunk(tmp, t ? t->format : 'o', t ? t->len : 4);
                free(tmp);
            }
            else
            {
                printchunk(line + i, t ? t->format : 'o', t ? t->len : 4);
            }
        }
        fputc('\n', stdout);
...
static int od(int fd, char* fname, int last)
{
...
static int lcm(unsigned int a, unsigned int b)
{
    unsigned int c, d, e;
...
static void addtype(char format, int len)
{
    struct type* t;
